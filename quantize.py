...
def _conv2d(x: Tensor, weight: Tensor, bias: Tensor = None, stride: int = 1, padding: int = 0) -> Tensor:
    ...
    output_h = (x.shape[2] - kernel_h) // stride + 1
    output_w = (x.shape[3] - kernel_w) // stride + 1
    ...
    for i in range(output.shape[2]):  # Hout
        for j in range(output.shape[3]):  # Wout
            ...
            h_pos, w_pos = slice(h_start, (h_start + kernel_h)), slice(w_start, (w_start + kernel_w))

            # 使用自定义近似乘法替换标准乘法
            output[:, :, i, j] = torch.sum(
                approx_mul(x[:, None, :, h_pos, w_pos], weight[None, :, :, :, :]), dim=(-3, -2, -1))

    return output + (bias[:, None, None] if bias is not None else 0)
...
class QuanNet(nn.Module):
    ...
    def quantize(self, training_data: Tensor, out_cover_f: float = 0.75):
        ...
        for i in range(len(module_list)):
            ...
            # 计算出权重的ws和wz，量化权重,收缩系数和零点根据tensor的最大最小值决定
...
class QuanLinear(nn.Linear):
    ...
    def _quan_forward(self, x: Tensor) -> Tensor:
        ...
        # torch.save(self.quan_weight,'/Users/feizhengpan/PycharmProjects/CNN_quantize/weight/' + generate_time_with_random_number() + '.pt')
